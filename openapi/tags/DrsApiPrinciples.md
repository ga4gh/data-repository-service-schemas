...
Prior to version 1.6, DRS was a read-only API. Read access is still the only behaviour that every implementation is expected to provide. An implementation MAY also support optional write and lifecycle operations: uploading and registering objects, adding checksums, updating access methods, and deleting objects. These operations are independent of one another, and different deployments will legitimately support different subsets depending on their role. A read-only mirror might support none of them, an ingest service might support upload and registration but not deletion, and a curated registry might support registration and metadata updates for data held elsewhere without accepting uploads. A server MAY support any combination of them, including none at all, and MUST accurately advertise which it supports through the corresponding capability flags in `/service-info`, each of which is described in the relevant appendix. Clients SHOULD rely on these flags rather than assume that support for one operation implies support for any other. These operations are purely additive: a server can adopt any of them without changing the read behaviour defined by earlier versions, so existing read-only clients continue to work unchanged. Implementations are still free to define their own mechanisms and interfaces for managing data beyond these.

We expect DRS objects to behave as stable records, whose identity and descriptive metadata do not change once they are registered. This is an ambition rather than something the protocol enforces. Where changes after registration are supported, they are limited to two cases. Access methods MAY be updated, because they point to a current storage location that can change as data is migrated. Checksums MAY be added, though servers are not expected to alter existing checksums, so that a consumer can attach an algorithm such as SHA-256 for content addressing without it having to be required at upload time. Object versioning and the restoration of deleted objects are out of scope here, and may be addressed in a future version of the spec.
