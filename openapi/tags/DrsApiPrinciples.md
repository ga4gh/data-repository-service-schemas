...
We expect DRS objects to behave as stable records, whose identity and descriptive metadata do not change once they are registered. Where changes after registration are supported, they are limited to two cases. Access methods MAY be updated, because they point to a current storage location that can change as data is migrated. Checksums MAY be added, though servers are not expected to alter existing checksums, so that a consumer can attach an algorithm such as SHA-256 for content addressing without it having to be required at upload time. Object versioning and the restoration of deleted objects are out of scope here, and may be addressed in a future version of the spec.
