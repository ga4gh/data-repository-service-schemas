...
Compact Identifier-based DRS URIs use meta-resolver services (specifically, https://identifiers.org/[identifiers.org] and https://n2t.net/[n2t.net (Name-To-Thing)]) to provide a layer of indirection between the DRS URI and the DRS server name -- the actual DNS name of the DRS server isn’t present in the URI. (See the <<_appendix_compact_identifier_based_uris>> for more background on Compact Identifiers and meta-resolver services.) They take the form:
