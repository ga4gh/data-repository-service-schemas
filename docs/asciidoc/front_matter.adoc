...
For convenience, including when passing content references to a WES server, we define a recommended URI syntax for DRS-accessible content. Strings of the form `drs://<server>/<id>` mean “_you can fetch the DRS content with id `<id>` from the DRS server at `<server>`_ ".
