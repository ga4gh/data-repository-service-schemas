...
* DRS objects can be referenced using two different URI conventions described below, one using compact identifiers following the conventions of identifiers.org/n2t.net and a second using hostnames
...
The prefix can be divided into an optional provider code and the namespace.  The accession here is an Ark, DOI, Data GUID, or other issuers's ID for the object being pointed to:
